...
environment variables — see app/settings.py. With ``AI_PROVIDER=bedrock``
(AWS, app/infra/bedrock_runner.py) or ``AI_PROVIDER=openai``
(app/infra/openai_runner.py), both passes run the same step workflow
(draft pass = steps 01–09, final pass = 10–11) on the chosen provider.
...
def generate_draft_items(export: dict, context_markdown: str,
    ...
    if settings.ai_configured() and provider in ("bedrock", "openai"):
        runner = _step_runner(provider)

        try:
            result = runner.run_workflow(
                "draft", export, prompts, documents, on_progress=on_progress)
            step09 = (result.project_data.get("workflow_run") or {}).get("09") or {}
            items = runner.extract_draft_items(
...
def _step_runner(provider: str):
    """The step-workflow runner module for a provider. Both expose the same
    surface (run_workflow / extract_draft_items / final_output)."""
    if provider == "openai":
        from app.infra import openai_runner
        return openai_runner
    from app.infra import bedrock_runner
    return bedrock_runner
...
# --------------------------------------------------------------------------- #
# Final pass — steps 10–11 via the step-runner (Bedrock or OpenAI)             #
# --------------------------------------------------------------------------- #

def _invoke_step_workflow(provider: str, export: dict, prompts: list[dict],
                          documents: list[dict], on_progress=None) -> dict:
    runner = _step_runner(provider)

    result = runner.run_workflow(
        "final", export, prompts, documents, on_progress=on_progress)
    out = runner.final_output(result.project_data)
    if not out:
        raise AIClientError(
    ...
    notes = (f"Generated by the {provider.title()} step workflow "
             f"(steps run: {', '.join(result.steps_run)}). "
             f"{result.usage.summary()}.")
    return {
        "ai_used": True,
...
def generate_tender(export: dict, context_markdown: str,
    ...
    if provider in ("bedrock", "openai"):
        try:
            return _invoke_step_workflow(provider, export, prompts, documents,
                                         on_progress=on_progress)
        except Exception as exc:  # noqa: BLE001 — degrade, don't dead-end
            LOGGER.exception("%s final pass failed; returning placeholders.",
                             provider)
            return _placeholder_result(
                export, context_markdown, prompts,
